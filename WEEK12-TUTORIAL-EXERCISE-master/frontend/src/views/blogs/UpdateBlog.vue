...
export default {
  ...
  created() {
    axios
      .get(`http://localhost:3002/blogs/${this.$route.params.id}`)
      .then((response) => {
  ...
  methods: {
    ...
    updateMainImage() {
      ...
      axios
        .put(
          `http://localhost:3002/image/setmain/${this.$route.params.id}/${this.selectMainId}`
        )
        .then((response) => {
    ...
    deleteCurrentImage(imageId) {
      ...
      if (comfirmDeleteImage == true) {
        axios
          .delete("http://localhost:3002/image/" + imageId)
          .then((response) => {
    ...
    submitBlog() {
      ...
      axios
        .put("http://localhost:3002/blogs/" + this.$route.params.id, formData)
        .then((res) => {
          console.log(res);
          this.$router.push({ path: "/" });
        })

          ...
          <div class="card">
            <div class="card-image">
              <figure class="image is-4by3">
                <img :src="'http://localhost:3001'+image.file_path" alt="Placeholder image" />
              </figure>
            </div>
...
export default {
  ...
  methods: {
    ...
    updateMainImage() {
      ...
      axios
        .put(
          `http://localhost:3001/image/setmain/${this.$route.params.id}/${this.selectMainId}`
        )
        .then((response) => {
    ...
    deleteCurrentImage(imageId) {
      ...
      if (comfirmDeleteImage == true) {
        axios
          .delete("http://localhost:3001/image/" + imageId)
          .then((response) => {
    ...
    submitBlog() {
      ...
      axios
        .put("http://localhost:3001/blogs/" + this.$route.params.id, formData)
        .then((res) => {
          console.log(res);
          this.$router.push({ path: "/" });
        })
